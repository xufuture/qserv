...
namespace {
    ...
    public:
         ...
         * an RPC protocol which runs an asynchronos loop (a chain) of two steps:
         * 
         *   - get a request
         *   - send back a reply
         * 
    ...
    private:
        ...
        /**
         * Begin reading (asynchronosly) a new request
         */
        void _read () {
            cout << "tcp_connection::    _read" << endl;

            async_read(_socket,
                       buffer(_messageReceived, 8),
        ...
        void handle_read (const boost::system::error_code& error,
            ...
            cout << "tcp_connection::      handle_read  ** bytes_transferred: " << bytes_transferred
                 << " _messageReceived: ";
            cout.write(_messageReceived.data(), bytes_transferred);
            cout << " ** " << endl;
            ...
            if (!strncmp(_messageReceived.data(), "DAYTIME", bytes_transferred)) {
                _write();
            } else {
                cout << "tcp_connection::      handle_read  ** unknown request: ";
                cerr.write(_messageReceived.data(), bytes_transferred);
                cerr << endl;
            }
        }
    ...
    private:
        ...
        boost::array<char, 9> _messageReceived;     // to accomodate 7 character of the operation name + trailing zero
        string                _messageSent;
    };
