...
/// be evaluated under the constraints of Qserv's shared nothing model so
/// long as the overlap partition contains all objects within 0.001 degrees
...
/// approach can almost double the memory required to hold an entire chunks
/// worth of sub-chunks in RAM. Since the worker wants chunks to fit entirely
...
/// queries. It may also slow down near-neighbor queries since it grows `k`
/// by the number of overlap rows. RIGHT joins have the same problem, as they
/// are equivalent to LEFT joins after commuting the left and right table
/// references.
///
...
/// about the partition of rows in one table based on the partition of rows in
/// another.
///
/// If this graph G is not connected, then there are two subgraphs G₁ and G₂
/// such that localizing the rows of relation G₁ to be within a single
/// partition and its overlap does not imply that the corresponding rows in G₂
/// share that partition. In other words, Qserv must assume that it cannot
/// evaluate the query using only worker-local data.
///
...
/// done using what is essentially a series of breadth first graph traversals:
///
...
///    within a partition; that is, no overlap is being used. Mark v as
///    visited and create an initially empty vertex queue Q.
///
/// 3. For each edge e incident to v leading to an as yet unvisited vertex u,
///    infer the partition of rows for u. In particular, the overlap oᵤ
///    required for u is determined from the overlap oᵥ required for v based
///    on the classification of e:
///
///    - director → director :
///    - director → child :
///    - director → match :
...
///    - match → child :
///
/// 4. Repeat step 3 on the vertices in Q until there are no more vertices
///    left to process.
///
/// 5. If all vertices are marked as visited after Q has been emptied, then
///    the query is evaluable (and the traversal has found and marked those
///    director vertices requiring overlap). Otherwise, choose a different
///    vertex from S, and repeat the process starting at step 2.
///
/// 6. If no vertex in S leads to a traversal that visits all vertices, then
///    the query is not evaluable by Qserv.
...
struct Vertex {
    ...
    Vertex(query::TableRef& t, TableInfo const* i) :
        tr(t), info(i), overlap(0.0), visited(false), queued(false) {}
...
public:
    ...
    /// table reference; it will contain a single vertex and no edges.
    RelationGraph(query::QueryContext const& ctx,
                  query::SelectStmt& stmt,
                  TableInfoPool& pool);

    /// `empty` tests whether this graph has no vertices.
...
private:
    ...
    size_t _makeOnEqEdges(
        query::BoolTerm::Ptr on,
        query::JoinRef::Type jt,
        RelationGraph& g);
    size_t _makeNaturalEqEdges(query::JoinRef::Type jt, RelationGraph& g);
    size_t _makeUsingEqEdges(
        query::ColumnRef const& c,
        query::JoinRef::Type jt,
        RelationGraph& g);
