...
// to 360.0 deg. Any input angle >= 360.0 - EPSILON_DEG is mapped to 360.0.
...
double clampRa(double ra) {
    if (ra >= 360.0 or (360.0 - ra < EPSILON_DEG)) {
        return 360.0;
    }
    return ra;
}
...
double SphericalTriangle::area() const {
    /* The area of a spherical triangle is it's spherical excess (see Girard's
       ...
       if required, an alternate formula is:
    ...
    return 2.0*M_PI - angSep(p20, p01) - angSep(p01, p12) - angSep(p12, p20);
}
...
/* The area of intersection between a spherical box and a spherical triangle
   ...
   decreases χ(M) by 1) and remains valid for non-convex surfaces.
   ...
   of the corresponding half-spaces. More generally, it is the angle between
   the vectors n₀ * v and n₁ * v, where * is the vector product. and
   n₀, n₁ are the plane normals of the half-spaces involved.
 */
...
class RaRangeList {
public:
    // A new RaRanges object contains a single range [-π,π].
    RaRangeList() : _numRanges(1) {
        _ranges[0].first = -M_PI;
        _ranges[0].second = M_PI;
    }
    ...
    // Remove all RA ranges from this list.
...
private:
    std::pair<double, double> _ranges[3];
    int _numRanges;
};

void RaRangeList::clip(double ra0, double ra1) {
    assert(ra0 != ra1);
    std::pair<double, double> out[3];
    int j = 0;
             ...
             double zmax)
{
    ...
    for (size_t i = 0, j = numVerts - 1; i < numVerts; j = i, ++i) {
        ...
        if (v > 0.0 && !bot.empty()) {
            ...
            // compute intersection points v0, v1 (unnormalized!)
    ...
    return 2.0*M_PI*chi - angle + top.extent()*zmax - bot.extent()*zmin;
}
...
double SphericalTriangle::intersectionArea(SphericalBox const & box) const {
    ...
    if (box.getRaMin() != 0.0 || box.getRaMax() != 360.0) {
        ...
        if (raExtent > 180.0 + EPSILON_DEG) {
            ...
            throw std::runtime_error("cannot compute area of intersection: "
                                     "spherical box has RA extent > 180 deg");
        }
        // intersect with half-space ra >= box.getRaMin()
        ...
        if (raExtent < 180.0 - EPSILON_DEG) {
            // intersect with half-space ra <= box.getRaMax()
