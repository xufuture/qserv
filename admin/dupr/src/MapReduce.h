...
///
/// Calls to `map` and `reduce` are performed in phases - essentially, `map` is
/// called over a pool of workers until either there is no input left or there
/// is no more memory to store `map` results in. At this point, the output
/// generated by `map` is consumed by having the pool of workers `reduce` it,
/// and the process repeats until the input is exhausted.
///
...
/// `reduce` calls receive. In addition, within a `reduce` phase, if a worker
/// sees a record with key K, then it is guaranteed to see all records with
/// that key. The end of each reduce phase is signalled by calling:
///
///     void finish();
///
...
/// is called. This is because the assignment of record keys to workers may
/// change during the next reduce phase. In the example, not flushing buffers
/// could lead to two different threads appending to the same file in parallel
/// and interleaving their data, thereby corrupting the file.
///
...
/// `boost::program_options::variables_map const &`, and define:
///
///     static void defineOptions(boost::program_options::options_description & opts)
///
/// where `defineOptions` is expected to define the configuration parameters
...
namespace detail {
    ...
    void JobBase<DerivedT, WorkerT>::_work() {
        ...
        while (true) {
            ...
            while (_sorted.size() != _numWorkers) {
                _reduceCond.wait(lock);
            }
            // Get a rank in [0, _numWorkers - 1) for this thread.
            uint32_t const rank = _rank;
            ++_rank;
            // Wake up next thread waiting to reduce (if any).
            ...
            lock.lock();
            // If no further input is available, store work results and
