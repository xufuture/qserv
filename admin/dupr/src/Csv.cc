...
// Character unescape lookup table.
uint8_t const Dialect::_unescape[256] = {
...
void Dialect::_validate() {
    ...
    for (int c = 1; c < 256; ++c) {
        uint8_t flag = 0;
        if (c == '\r')            { flag = HAS_CRLF;   }
        else if (c == '\n')       { flag = HAS_CRLF;   }
        else if (c == _delimiter) { flag = HAS_DELIM;  }
        else if (c == _quote)     { flag = HAS_QUOTE;  }
        else if (c == _escape)    { flag = HAS_ESCAPE; }
        _scanLut[c] = flag;
    }
...
char const * Editor::readRecord(char const * beg, char const * end) {
    if (end <= beg || beg == 0) {
    ...
    for (; beg < end; ++beg) {
        ...
        if (escaped) {
            ...
            if (c == _inputDialect.getEscape()) {
                ...
                f->inputValue = beg + 1;
                if (beg + 1 < end && beg[1] == _inputDialect.getQuote()) {
...
// There is a subtlety here. Since these specializations invoke strtoXXX(),
...
// converted. This can lead to crashes or incorrect results - consider the case
// where the field delimiter is a digit.

#define GET_PREAMBLE(U) \
    ...
    if (sz == 0) { \
        throw runtime_error("Cannot convert empty field value to a C++ " #U); \
    } \
