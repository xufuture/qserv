...
docker run -e "constraint:node==$MASTER" --detach=true -e "QSERV_MASTER=$MASTER" --name "$MASTER" --net=host "$MASTER_IMAGE"

for i in $WORKERS;
do
    docker rm -f "$i" || echo "No existing container for $i"
    docker run -e "constraint:node==$i" --detach=true -e "QSERV_MASTER=$MASTER" --name "$i" --net=host  "$WORKER_IMAGE"
