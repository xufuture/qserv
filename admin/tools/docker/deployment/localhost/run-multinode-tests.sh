...
docker run --detach=true --dns="$DNS_IP" --dns-search="$DNS_DOMAIN" -e "XROOTD_MANAGER_HOSTNAME=$MASTER" --name "$MASTER" -h "${MASTER}" "$MASTER_IMAGE"

for i in $WORKERS;
do
    docker rm -f "$i" || echo "No existing container for $i"
    docker run --detach=true --dns="$DNS_IP" --dns-search="$DNS_DOMAIN" -e "XROOTD_MANAGER_HOSTNAME=$MASTER" --name "$i" -h "${i}"  "$WORKER_IMAGE"
done
